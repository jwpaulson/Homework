...
Problem Number 6)\\
We have the situation where F is less than that given by (10.2) Thus to see the arbitrage, we first buy and go long one unit of a forward contact with forward price F. Next we short 1 unit spot which delivers cash s at time zero. To execute this short, we borrow the asset and collect cash flows for the carrying costs. We then invest these cash flows by lending them out. Finally we need to buy one unit due to our forward and then return it as we orignially borrowed it. The table below, sums this up and shows the arbitrage.
...
Short 1 unit & -s & 0 & 0 \\
Borrow asset & -c(0) & -c(k) & 0 \\
Lend \$\$ Invest cash flow & c(0) & c(k) & $\sum_{k=0}^{M-1} \frac{c(k)}{d(k,M)}$ \\
Buy 1 unit & s & 0 & $\frac{S(0,M)}{d(0,M)}$ \\ \hline
